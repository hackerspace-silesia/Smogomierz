...
bool loadDeviceConfig() {
  ...
  if (!configFile) {
    Serial.println("Failed to open device_config file");
    return false;
  }

  size_t size = configFile.size();
  if (size > 1536) {
    Serial.println("services_config file size is too large");
    return false;
  }
  ...
  if (json.isNull()) {
    Serial.println("Failed to parse services_config file");
    return false;
  }
...
bool loadServicesConfig() {
#ifdef ARDUINO_ARCH_ESP8266
  File configFile = SPIFFS.open("/services_config.json", "r");
#elif defined ARDUINO_ARCH_ESP32
  File configFile = SPIFFS.open("/services_config.json");
#endif
  
  if (!configFile) {
    Serial.println("Failed to open services_config file");
    return false;
  }
...
bool saveDeviceConfig() {
    ...
    if (!configFile) {
      Serial.println("Failed to open device_config file for writing");
      return false;
    }
...
bool saveServicesConfig() {
    ...
    File configFile = SPIFFS.open("/services_config.json", "w");
    if (!configFile) {
      Serial.println("Failed to open services_config file for writing");
      return false;
    }
...
void fs_setup() {
  ...
  */
  if (!loadDeviceConfig()) {
    Serial.println("Failed to load device_config");
    Serial.println("Saving the current device_config...");
    saveDeviceConfig();
  } else {
    Serial.println("device_config loaded");
  }
  
  if (!loadServicesConfig()) {
    Serial.println("Failed to load services_config");
    Serial.println("Saving the current services_config...");
    saveServicesConfig();
  } else {
    Serial.println("services_config loaded");
  }
}
...
void deleteConfig() {
  ...
  SPIFFS.remove("/services_config.json");
}